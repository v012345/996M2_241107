local XinYueMiBaoGe2 = {}

--接收请求
function XinYueMiBaoGe2.Request(actor, var)
    if var == 1 then
    ...
    if var == 2 then
        ...
        XinYueMiBaoGe2.SyncResponse(actor)
    end

end
--同步消息
function XinYueMiBaoGe2.SyncResponse(actor, logindatas)
    ...
    local _login_data = {ssrNetMsgCfg.XinYueMiBaoGe2_SyncResponse, TongXingZheng, RiChongNum, 0, nil}
    if type(logindatas) == "table" then
        table.insert(logindatas, _login_data)
    else
        Message.sendmsg(actor, ssrNetMsgCfg.XinYueMiBaoGe2_SyncResponse, TongXingZheng, RiChongNum, 0, nil)
    end
end
--登录触发
local function _onLoginEnd(actor, logindatas)
    XinYueMiBaoGe2.SyncResponse(actor, logindatas)
end
--事件派发
GameEvent.add(EventCfg.onLoginEnd, _onLoginEnd, XinYueMiBaoGe2)
--注册网络消息
Message.RegisterNetMsg(ssrNetMsgCfg.XinYueMiBaoGe2, XinYueMiBaoGe2)
return XinYueMiBaoGe2