local ShengChengMiBaoGeB = {}
---------------网络消息----------------
function ShengChengMiBaoGeB.Request(actor, var)
    ...
    if var == 2 then
        ...
        ShengChengMiBaoGeB.SyncResponse(actor)
    end
...
--注册网络消息
function ShengChengMiBaoGeB.SyncResponse(actor, logindatas)
    ...
    local _login_data = { ssrNetMsgCfg.ShengChengMiBaoGeB_SyncResponse, TongXingZheng, RiChongNum, 0, nil }
    if type(logindatas) == "table" then
        table.insert(logindatas, _login_data)
    else
        Message.sendmsg(actor, ssrNetMsgCfg.ShengChengMiBaoGeB_SyncResponse, TongXingZheng, RiChongNum, 0, nil)
    end
end
Message.RegisterNetMsg(ssrNetMsgCfg.ShengChengMiBaoGeB, ShengChengMiBaoGeB)

--登录触发
local function _onLoginEnd(actor, logindatas)
    ShengChengMiBaoGeB.SyncResponse(actor, logindatas)
end
GameEvent.add(EventCfg.onLoginEnd, _onLoginEnd, ShengChengMiBaoGeB)


return ShengChengMiBaoGeB
