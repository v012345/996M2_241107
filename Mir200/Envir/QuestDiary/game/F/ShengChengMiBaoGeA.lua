local ShengChengMiBaoGeA = {}
---------------网络消息----------------
function ShengChengMiBaoGeA.Request(actor, var)
    ...
    if var == 2 then
        ...
        ShengChengMiBaoGeA.SyncResponse(actor)
    end
...
--注册网络消息
function ShengChengMiBaoGeA.SyncResponse(actor, logindatas)
    ...
    local _login_data = { ssrNetMsgCfg.ShengChengMiBaoGeA_SyncResponse, TongXingZheng, RiChongNum, 0, nil }
    if type(logindatas) == "table" then
        table.insert(logindatas, _login_data)
    else
        Message.sendmsg(actor, ssrNetMsgCfg.ShengChengMiBaoGeA_SyncResponse, TongXingZheng, RiChongNum, 0, nil)
    end
end
Message.RegisterNetMsg(ssrNetMsgCfg.ShengChengMiBaoGeA, ShengChengMiBaoGeA)

--登录触发
local function _onLoginEnd(actor, logindatas)
    ShengChengMiBaoGeA.SyncResponse(actor, logindatas)
end
GameEvent.add(EventCfg.onLoginEnd, _onLoginEnd, ShengChengMiBaoGeA)



return ShengChengMiBaoGeA
