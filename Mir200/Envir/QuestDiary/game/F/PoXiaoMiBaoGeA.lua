local PoXiaoMiBaoGeA = {}
---------------网络消息----------------
function PoXiaoMiBaoGeA.Request(actor, var)
    ...
    if var == 2 then
        ...
        PoXiaoMiBaoGeA.SyncResponse(actor)
    end

end
-------------网络消息↓↓↓--------------------------
--注册网络消息
function PoXiaoMiBaoGeA.SyncResponse(actor, logindatas)
    ...
    local _login_data = { ssrNetMsgCfg.PoXiaoMiBaoGeA_SyncResponse, TongXingZheng, RiChongNum, 0, nil }
    if type(logindatas) == "table" then
        table.insert(logindatas, _login_data)
    else
        Message.sendmsg(actor, ssrNetMsgCfg.PoXiaoMiBaoGeA_SyncResponse, TongXingZheng, RiChongNum, 0, nil)
    end
end
Message.RegisterNetMsg(ssrNetMsgCfg.PoXiaoMiBaoGeA, PoXiaoMiBaoGeA)

--登录触发
local function _onLoginEnd(actor, logindatas)
    PoXiaoMiBaoGeA.SyncResponse(actor, logindatas)
end
GameEvent.add(EventCfg.onLoginEnd, _onLoginEnd, PoXiaoMiBaoGeA)



return PoXiaoMiBaoGeA
