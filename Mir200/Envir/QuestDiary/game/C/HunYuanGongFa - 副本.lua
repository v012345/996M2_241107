...
function HunYuanGongFa.Request(actor, arg1)
    ...
    if getplaydef(actor,VarCfg["U_混元功法等级"]) == arg1*10 then
        local Skillid = nil
        Skillid = getskillindex(config[arg1*10].skillName)
        addskill(actor, Skillid, 3)
        if cfg.skillName == "神之开天" or cfg.skillName == "灭神之逐"    then
            local Skillidx = (cfg.skillName == "灭神之逐" and 56) or (cfg.skillName == "神之开天" and 66)
